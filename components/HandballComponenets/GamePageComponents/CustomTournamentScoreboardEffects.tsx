...
export function customTournamentScoreboardEffects(game?: GameStructure) {
    ...
    switch (events.at(-1).eventType) {
        case 'Score':
            switch (game.events.at(-1).notes) {
                case 'Ace':
                    return (
                ...
                default:
                    ...
                    const minPos = 0;
                    const pos = `calc(50% ${game.events.at(-1).firstTeam ? '-' : '+'} ${fakeRandom(game.changeCode) * (maxPos - minPos) + minPos}px)`;
                    ...
                    return (
                        <Box pos="absolute" w="100vw" h="100vh" style={{ overflow: 'hidden' }}>
                            <Image
                              ...
                              style={{ position: 'absolute', left: pos, top: '50%' }}></Image>
                        </Box>
                    );
            }
    }
